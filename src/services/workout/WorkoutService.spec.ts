  ...
  const { mocks: {loggerMock}} = createLoggerMock();
    ...
    describe('get workouts', () => {
        ...
        describe('could not retrieve workouts data', () => {
          ...
          beforeEach(() => {
              workoutDaoImplementationMock.getWorkouts.mockRejectedValueOnce(new Error(errorMessage));
            });

            it('should log the error',  async () => {
              await expect(cut.getWorkouts()).rejects.toThrow(errorMessage);
              expect(loggerMock.error).toHaveBeenCalledWith(`WorkoutService:: getWorkouts: Error: ${errorMessage}`);
            });
        });
    });
    ...
    describe('createWorkout', () => {
      ...
      const workout: Workout = {
        ...
        it('should call workoutDao.createWorkout', async () => {
            ...
            expect(workoutDaoImplementationMock.createWorkout).toHaveBeenCalledWith(workout);
        });

      describe('could not create workouts data', () => {
        const errorMessage = 'Failed to save the data.';
        beforeEach(() => {
          workoutDaoImplementationMock.createWorkout.mockRejectedValueOnce(new Error(errorMessage));
        });

        it('should throw an error',  async () => {
          await expect(cut.createWorkout(workout)).rejects.toThrow(errorMessage);
          expect(loggerMock.error).toHaveBeenCalledWith(`WorkoutService:: createWorkout: Error: ${errorMessage}`);
        });
      });
    });
    ...
    describe('getWorkoutId', () => {
      ...
      it('should call workoutDao.getWorkout', async () => {
            ...
            expect(workoutDaoImplementationMock.getWorkout).toHaveBeenCalledWith(workoutId);
        });

      describe('could not get workout by id', () => {
        const errorMessage = 'Failed to get workout Id.';
        beforeEach(() => {
          workoutDaoImplementationMock.getWorkout.mockRejectedValueOnce(new Error(errorMessage));
        });

        it('should throw an error',  async () => {
          await expect(cut.getWorkout(workoutId)).rejects.toThrow(errorMessage);
          expect(loggerMock.error).toHaveBeenCalledWith(`WorkoutService:: getWorkoutId: Error: ${errorMessage}`);
        });
      });
    });
    ...
    describe('deleteWorkout', () => {
      ...
      it('should call workoutDao.deleteWorkout', async () => {
            ...
            expect(workoutDaoImplementationMock.deleteWorkout).toHaveBeenCalledWith(workoutId);
        });

      describe('could not get delete an workout Id', () => {
        const errorMessage = 'Failed to delete workout Id.';
        beforeEach(() => {
          workoutDaoImplementationMock.deleteWorkout.mockRejectedValueOnce(new Error(errorMessage));
        });

        it('should throw an error',  async () => {
          await expect(cut.deleteWorkout(workoutId)).rejects.toThrow(errorMessage);
          expect(loggerMock.error).toHaveBeenCalledWith(`WorkoutService:: deleteWorkouts: Error: ${errorMessage}`);
        });
      });
    });
});
